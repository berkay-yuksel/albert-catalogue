...
function OrderItemRow({
  ...
  function commit() {
    ...
    if (clamped !== qty) onSetQty(itemKey, clamped);
  }
      ...
      <div className="order-item-info">
        ...
        {product && (
          <div className="order-item-price">
            {linePrice > 0 ? (
              <>
                {fmtPrice(linePrice)} × {qty} = <b>{fmtPrice(linePrice * qty)}</b>
              </>
            ) : (
        ...
        <div className="order-item-qty">
          ...
          <input
            ...
            aria-label="Quantity"
          />
          <button onClick={() => onChangeQty(itemKey, 1)} aria-label="Increase quantity">
            +
          </button>
        </div>
      </div>
      <button className="order-item-remove" onClick={() => onRemove(itemKey)} aria-label="Remove">
        ✕
      </button>
    </div>
  );
...
export function OrderPanel({
  ...
  const hasFineJewelryItem = Object.values(cart.items).some((line) => {
  ...
  function sendOrder() {
    ...
    const mailto = buildOrderMailto(cart.items, products, cart.linePrice, { notes });
  ...
  async function copyOrder() {
    if (keys.length === 0) return;
    const text = buildOrderText(cart.items, products, cart.linePrice, { notes });
    try {
        ...
        {keys.length > 0 && (
          <div className="order-extras">
            ...
            <textarea
              ...
              placeholder="e.g. preferred delivery timeline, custom requests..."
              value={notes}
